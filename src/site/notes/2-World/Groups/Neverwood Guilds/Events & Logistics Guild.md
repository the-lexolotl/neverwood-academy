---
{"tags":["Category/Group"],"MyContainer":"[Neverwood Academy]]","MyCategory":"Student Guild","image":"Template_Galaxy_Placeholder.png","obsidianUIMode":"preview","leaders":null,"staff":null,"members":null,"initiates":null,"primary_contact":"[[Jasper Fairweather](/img/user/2-World/Hubs/Neverwood%20Academy.md)","Skill1":["Insight"],"Skill2":["Persuasion"],"Skill3":["Investigation"],"dg-publish":true,"dg-show-local-graph":true,"dg-show-inline-title":true,"dg-show-backlinks":true,"dg-show-toc":true,"guildType":"Student Guild","dg-path":"World/Groups/Neverwood Guilds/Events & Logistics Guild.md","permalink":"/world/groups/neverwood-guilds/events-and-logistics-guild/","dgShowBacklinks":true,"dgShowLocalGraph":true,"dgShowInlineTitle":true,"dgShowToc":true,"dgPassFrontmatter":true,"dg-note-properties":{"tags":["Category/Group"],"MyContainer":"[Neverwood Academy]]","MyCategory":"Student Guild","image":"Template_Galaxy_Placeholder.png","obsidianUIMode":"preview","leaders":null,"staff":null,"members":null,"initiates":null,"primary_contact":"[[Jasper Fairweather](/img/user/2-World/Hubs/Neverwood%20Academy.md)","Skill1":["Insight"],"Skill2":["Persuasion"],"Skill3":["Investigation"],"guildType":"Student Guild"}}
---


> [!infobox|left]+
> # Events & Logistics Guild
> ![Events & Logistics.png](/img/user/z_Assets/Posters/Events%20&%20Logistics.png)
> ###### General Information
> Motto:  |  “Precision, Planning, Perfection.” |
> ---|---|
> **Location:** | [[2-World/Hubs/Neverwood Academy\|Neverwood Academy]] | 
> **Category:** | Student Guild |
> **Primary Contact:** | [[2-World/People/Neverwood Academy/Students/1506/Jasper Fairweather\|Jasper Fairweather]] |
> **Skill 1:** | Insight |
> **Skill 2:** | Persuasion |
> **Skill 3:** | Investigation |

# Overview
The Events & Logistics Guild ensures that life at Neverwood runs smoothly, coordinating everything from grand festivals to small student gatherings with precision and efficiency.

# Player Information
### Base Pay
6gp per tenday
### Job Pay 
Event-based; larger or more complex events provide greater rewards
### Async Commitment
1 async task per term (event planning, coordination, or crisis resolution).
### Group Cup Opportunities
- Successfully running events
- Preventing or resolving crises
- Enhancing campus life 
- Creative contributions

# Purpose
- Plan and execute campus events
- Coordinate logistics across departments and guilds
- Manage crowds, schedules, and resources
- Resolve operational issues and emergencies

# Membership
Membership is open to second-year students and above.
Applicants must demonstrate strong organisational skills and the ability to remain composed under pressure.
All applications are reviewed at the beginning of the academic year. Positions are limited due to the high level of responsibility entrusted to members.

# Roles & hierarchy 
- **Junior Coordinator** – Assists with planning and setup
- **Logistics Operative** – Manages specific aspects of events
- **Event Coordinator** – Oversees full event execution
- **Crisis Manager** – Handles unexpected disruptions
- **Guild Director (Student Lead)** – Leads planning efforts and liaises with faculty

# Perks
- Influence over major academy events
- Strong connections with faculty and visiting figures
- Opportunities to shape campus culture
- Consistent Group Cup point opportunities
- Recognition for successful large-scale operations

# Description
The Events & Logistics Guild is responsible for the seamless execution of life at [[2-World/Hubs/Neverwood Academy\|Neverwood Academy]]. From grand festivals and ceremonial gatherings to smaller student-led functions, the guild ensures that every detail is accounted for.
Established alongside the academy’s earliest traditions, the guild has become an essential pillar of campus life. Its members operate behind the scenes, coordinating schedules, managing resources, and resolving crises before they can disrupt proceedings.
Despite its polished exterior, the guild thrives in controlled chaos, where quick thinking and adaptability are just as valuable as careful planning.

# People
The following people are members of this guild.  
| File                                                                                                                         | Gender     | Race      | Role    | College     | School Year |
| ---------------------------------------------------------------------------------------------------------------------------- | ---------- | --------- | ------- | ----------- | ----------- |
| [[2-World/People/Neverwood Academy/Students/1506/Jasper Fairweather\|Jasper Fairweather]]                                 | Male       | Satyr     | Student | Prismari    | 4           |
| [[2-World/People/Neverwood Academy/Students/1506/Nyx Veldrith\|Nyx Veldrith]]                                             | Non-binary | Changling | Student | Silverquill | 4           |
| [[2-World/People/Neverwood Academy/Students/1507/Derthag Shadowslayer\|Derthag Shadowslayer]]                             | Non-binary | Orc       | Student | Prismari    | 3           |
| [[2-World/People/Neverwood Academy/Students/1507/Drazhomir Yarnask\|Drazhomir Yarnask]]                                   | Male       | Minotaur  | Student | Quandrix    | 3           |
| [[2-World/People/Neverwood Academy/Students/1508/Aurora Luna Wyntherstarr\|Aurora Luna Wyntherstarr]]                     | Female     | Dhampir   | Student | Witherbloom | 2           |
| [[2-World/People/Neverwood Academy/Students/1508/Quentillius Antiphiun Melentor III\|Quentillius Antiphiun Melentor III]] | Male       | Human     | Student | Prismari    | 2           |

{ .block-language-dataview}

