---
{"MyContainer":["[Witherbloom Campus]]","[[Neverwood Academy|Neverwood Academy]]"],"MyCategory":null,"image":"Pasted image 20250911160709.png","tags":["Category/People"],"obsidianUIMode":"preview","aliases":null,"NoteStatus":"❓","char_status":"Alive","char_race":"Human","char_gender":"Male","char_role":"Veteran","char_college":"Witherbloom","char_items":null,"char_age":"Elder","parents":null,"children":["[[2-World/People/Neverwood Academy/Staff/Witherbloom/Miaukier\|Miaukier](/img/user/2-World/Places/Witherbloom%20Campus.md)"],"enemies":null,"allies":null,"siblings":null,"partner":null,"Connected_Quests":["[Miaukier Must Die!]]"],"Connected_Groups":["[[2-World/Groups/Neverwood Staff\|Neverwood Staff]]","[[Witherbloom College|Witherbloom College]]"],"dg-publish":true,"dg-show-local-graph":true,"dg-show-inline-title":true,"dg-show-backlinks":true,"dg-path":"World/People/Neverwood Academy/Staff/Witherbloom/Theodore Moss.md","permalink":"/world/people/neverwood-academy/staff/witherbloom/theodore-moss/","dgShowBacklinks":true,"dgShowLocalGraph":true,"dgShowInlineTitle":true,"dgPassFrontmatter":true,"dg-note-properties":{"MyContainer":["[[2-World/Places/Witherbloom Campus.md|Witherbloom Campus]]","[[Neverwood Academy|Neverwood Academy]]"],"MyCategory":null,"image":"Pasted image 20250911160709.png","tags":["Category/People"],"obsidianUIMode":"preview","aliases":null,"NoteStatus":"❓","char_status":"Alive","char_race":"Human","char_gender":"Male","char_role":"Veteran","char_college":"Witherbloom","char_items":null,"char_age":"Elder","parents":null,"children":["[[2-World/People/Neverwood Academy/Staff/Witherbloom/Miaukier\|Miaukier](/img/user/2-Quests/Miaukier%20Must%20Die!.md)"],"enemies":null,"allies":null,"siblings":null,"partner":null,"Connected_Quests":["[[Miaukier Must Die!|Miaukier Must Die!]]"],"Connected_Groups":["[[2-World/Groups/Neverwood Staff\|Neverwood Staff]]","[[Witherbloom College|Witherbloom College]]"]}}
---


> [!infobox|left]+
> # Theodore Moss
> ![Pasted image 20250911160709.png](/img/user/z_Assets/character_art/NPCs/Pasted%20image%2020250911160709.png)
> ###### Basic Info
> Type |  Stat |
> ---|---|
> Status | Alive |
> Race | Human |
> Gender | Male |
> Age | Elder |
> Location | [[2-World/Places/Witherbloom Campus\|Witherbloom Campus]], [[2-World/Hubs/Neverwood Academy\|Neverwood Academy]] |
> Role | Veteran |
> College | Witherbloom |

# Biography
Theodore Moss is a venerable and eccentric potions master who once served as a Professor of Growth within Witherbloom College. Over a long and distinguished career, he became known for his unconventional methods, blending alchemy, herbalism, and intuition into potions whose recipes were rarely written down and never brewed the same way twice. Students often recall lessons that wandered far from the syllabus, guided instead by Theodore’s curiosity and fondness for experimentation.

Though long retired, Theodore continues to reside on campus, tending to his gardens and quietly contributing to the academy’s atmosphere of gentle oddity. He is rarely seen without his beloved companion, [[2-World/People/Neverwood Academy/Staff/Witherbloom/Miaukier\|Miaukier]], whose unusual draconic form has done nothing to diminish the affection between them. To those who know him well, Theodore Moss remains a living reminder that wisdom does not fade with age—it simply grows more idiosyncratic.

# Connections

**Quests:** [[2-Quests/Miaukier Must Die!\|Miaukier Must Die!]]
**Groups:**  [[2-World/Groups/Neverwood Staff\|Neverwood Staff]],[[2-World/Groups/Colleges/Witherbloom College\|Witherbloom College]]

### Associated Handouts
| File | Date Obtained |
| ---- | ------------- |

{ .block-language-dataview}


# Relationships
**Parents** - None/Unknown
**Siblings** - None/Unknown
**Partner** - None/Unknown
**Children** - [[2-World/People/Neverwood Academy/Staff/Witherbloom/Miaukier\|Miaukier]]
**Allies** - None/Unknown
**Enemies** - None/Unknown

