---
{"tags":["Category/Quest"],"MyContainer":["[[2-World/Hubs/Neverwood Academy.md|Neverwood Academy]]","[[2-World/Places/Quandrix Campus.md|Quandrix Campus]]"],"MyCategory":null,"image":"Template_Quest_Placeholder.png","obsidianUIMode":"preview","questObtained":"2025-10-24","questStatus":"In Progress","questGiver":"[[2-World/People/Other/Professor Delta.md|Professor Delta]]","questLocationObtained":"[[2-World/Places/Quandrix Campus.md|Quandrix Campus]]","questSessionObtained":"[[1-Session Journals/2025-10-24 - Ross Async.md|2025-10-24 - Ross Async]]","questNotes":null,"questLootAvail":null,"questLookEarned":null,"NoteIcon":"quest","dg-publish":true,"dg-path":"World/Quests/Sigma Mail.md","permalink":"/world/quests/sigma-mail/","dgPassFrontmatter":true,"updated":"2025-10-24T22:03:26.000+01:00"}
---




# Quest Info
### Initial Steps – Delivery After Hours
- [x] ~~Receive the envelope from [[2-World/People/Students/1508/Cadoras Damellawar\|Cadoras Damellawar]] in the library.~~
- [x] ~~Inspect the contents: note, platinum, and sigil.~~
- [x] ~~Decide whether to go to the meeting at the old observatory.~~

### Meeting the Representative
- [x] ~~Arrive at the observatory at midnight.~~
- [x] ~~Meet [[2-World/People/Other/Professor Delta\|"Professor Delta"]], the representative of The Silent Equation.~~
- [x] ~~Accept the assignment: decode the ancient spellbook and copy its spells onto scrolls.~~
- [x] ~~Take note of the unusual properties of the book.~~

### Decoding the Spellbook
- [ ] Examine each page of the book and solve its mathematical-magical puzzles. Pages include:
	- [x] ~~The Law of Balanced Force~~
	- [ ] Constant Zero
	- [ ] Recursive Function
	- [ ] Mean of Averages
	- [ ] Fractal Bloom
	- [ ] Limit Approaching Zero
- [ ] Choose whether to test, copy, or modify each spell.

### Scroll Creation 
- [ ] Copy spells onto scrolls.
- [ ] Handle any unexpected magical phenomena from the book:
	- [ ] Ink rearranging itself.
	- [ ] Shadow lines appearing on the page.
	- [ ] Whispers of mathematical terms.

### Moral Choice and Consequences
- [ ] Decide whether to follow instructions faithfully, alter the spells, or refuse the task.

# Quest Details


| Date Obtained:    | 2025-10-24                             |
| ----------------- | -------------------------------------- |
| Status:           | In Progress                            |
| Quest Giver:      | [[2-World/People/Other/Professor Delta\|"Professor Delta"]] |
| Quest Location:   | [[2-World/Places/Quandrix Campus\|Quandrix Campus]]                    |
| Session Obtained: | [[1-Session Journals/2025-10-24 - Ross Async\|2025-10-24 - Ross Async]]            |
| Acquired Loot:    |                                        |
# Backstory

[[2-World/People/Students/1508/Cadoras Damellawar\|Cadoras Damellawar]] approaches [[1-Party/Group 2/Harry Blackstone\|Harry]] after receiving a mysterious payment on behalf of an organization called [[2-World/Groups/The Silent Equation\|The Silent Equation]]. The envelope contains platinum and a request: meet their representative at the old observatory to assist in decoding an ancient, coded spellbook. The task seems simple at first, but the nature of the book and the organization is unclear — and the opportunity is too intriguing to ignore.


# People

The following people are associated with this quest.

| File | Race | Gender | Role |
| ---- | ---- | ------ | ---- |

{ .block-language-dataview}
